(function (L) {
    ...
    _this = L.Topic = {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.success) {
                        ...
                        _this.loadRelateds(result.data.topic);
                        _this.loadComments();
                        _this.initEvents();
        ...
        loadRelateds: function (topic) {
            if (!topic) {
                return false
            }
            $.ajax({
                url: '/topic/' + _this.data.topic_id + '/relateds',
                type: 'get',
                cache: false,
                data: {
                    title: topic.title,
                    albumId: topic.albumId,
                    epindex: topic.epindex
                },
                dataType: 'json',
                success: function (result) {
                    if (result.success) {
                        if (!result.data || (result.data && result.data.relateds.length <= 0)) {
                            $("#relateds-body").html('<div class="alert alert-info" role="alert">没有任何推荐内容</div>')
                        } else {
                            var $container = $("#relateds-body");
                            $container.empty();
                            var tpl = $("#topic-related-tpl").html();
                            var html = juicer(tpl, result.data);
                            $container.html(html);
                            $("#topic-related").show();
                        }
                    } else {
                        $("#relateds-body").html('<div class="alert alert-danger" role="alert">' + result.msg + '</div>');
                    }
                },
                error: function () {
                    $("#relateds-body").html('<div class="alert alert-danger" role="alert">发送请求失败.</div>');
                }
            });
        },
