...
if resp and resp.hits then
	...
	for _,v in ipairs(resp.hits.hits) do
		...
		if meta then
			if not meta.sort then
			   meta.sort = 0
			   if string.len(meta.id) > 16 then
			   	  meta.sort =1
			   end
			end
			local sortName = util_const.index2Name("SORT_DICT",meta.sort)
